module maze_game::game_tokens {
    ...
    public entry fun transfer_tokens(
        ...
        let from_tokens = borrow_global_mut<GameTokens>(from_addr);
        assert!(from_tokens.tokens >= amount, error::insufficient_balance(EINSUFFICIENT_BALANCE));

        // Ensure recipient account exists
        assert!(exists<GameTokens>(to_address), error::invalid_argument(ENOT_ENOUGH_TOKENS));

        let to_tokens = borrow_global_mut<GameTokens>(to_address);

        // Transfer tokens
        from_tokens.tokens = from_tokens.tokens - amount;
        to_tokens.tokens = to_tokens.tokens + amount;
    ...
    public entry fun execute_sabotage(
        ...
        assert!(tokens.tokens >= cost, error::insufficient_balance(EINSUFFICIENT_BALANCE));

        // Check cooldown
        let cooldown_end = get_sabotage_cooldown_end(cooldowns, sabotage_type);
        assert!(current_timestamp >= cooldown_end, error::invalid_argument(COOLDOWN_ACTIVE));
